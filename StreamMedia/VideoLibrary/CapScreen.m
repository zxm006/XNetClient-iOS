...
//        DLog(@"KeyFrame == %lu",(unsigned long)[Bytedata length]);
    }
    else
    {
        [Bytedata setData:ByteHeader];
        [Bytedata appendData:data];
//         DLog(@"p Frame == %lu",(unsigned long)[Bytedata length]);
        //        DLog(@"isKeyFrame=no");
    }
    
    
    
    if(m_MediaScreenVideoCallBack)
    {
        
        m_MediaScreenVideoCallBack->On_MediaReceiverCallbackScreenVideo((unsigned char*)[Bytedata bytes],(int)[Bytedata length],isKeyFrame , m_uiWidth, m_uiHeight);
    }
    
}


-(void)SetCallBack:(MediaScreenVideoCallBack*)pMediaScreenVideoCallBack
{
...
//         CGContextRef bitmapContext = [self createPixelBufferAndBitmapContext:&pixelBuffer];
        ...
        if([keyWindow isKindOfClass:NSClassFromString(@"_UIAlertControllerShimPresenterWindow")])
        {
          UIGraphicsPushContext(bitmapContext);
            [keyWindow drawViewHierarchyInRect:CGRectMake(0, 0, keyWindow.frame.size.width, keyWindow.frame.size.height) afterScreenUpdates:NO];
            
            NSArray *windows = [[UIApplication sharedApplication] windows];
         
              [windows[0] drawViewHierarchyInRect:CGRectMake(0, 0, keyWindow.frame.size.width, keyWindow.frame.size.height) afterScreenUpdates:NO];
