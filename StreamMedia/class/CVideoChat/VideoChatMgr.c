...
{
    if (m_VideoChatDelegate &&[m_VideoChatDelegate respondsToSelector:@selector(VideoChatInfoReg:result:info:)]) {
        
        dispatch_async(dispatch_get_main_queue(), ^{
            
        [m_VideoChatDelegate VideoChatInfoReg:mtgCmd result:result info:info];
        });
    }
...
{
    if (m_VideoChatDelegate &&[m_VideoChatDelegate respondsToSelector:@selector(VideoChatInfo:username:info:)]) {
        dispatch_async(dispatch_get_main_queue(), ^{
            
        [m_VideoChatDelegate VideoChatInfo:mtgCmd username:username info:info];
        });
    }
...
{
    if (m_VideoChatDelegate &&[m_VideoChatDelegate respondsToSelector:@selector(VideoChatInvite:mtgId:)]) {
        dispatch_async(dispatch_get_main_queue(), ^{
            
        [m_VideoChatDelegate VideoChatInvite:username mtgId:mtgId];
        });
    }
}
...
{
    if (m_VideoChatDelegate &&[m_VideoChatDelegate respondsToSelector:@selector(VideoChatJoin:)]) {
        dispatch_async(dispatch_get_main_queue(), ^{
            
            [m_VideoChatDelegate VideoChatJoin:userinfo];
        });
    }
}
  void IVideoChat::MtgExit(CLIENTUSERINFOLIST &userinfo)
{
    if (m_VideoChatDelegate &&[m_VideoChatDelegate respondsToSelector:@selector(VideoChatExit:)]) {
        dispatch_async(dispatch_get_main_queue(), ^{
            
            [m_VideoChatDelegate VideoChatExit:userinfo];
        });
    }
...
{
    if (m_VideoChatDelegate &&[m_VideoChatDelegate respondsToSelector:@selector(VideoChatUserList:)]) {
      [m_VideoChatDelegate VideoChatUserList:UserInfoList];
...
void loginVideoChatServer(const char* szServerHostURL,const char* szUserName, const char* headUrl,
                    const char* nickName,double latitude,double longitude,const char* gameid,bool isencrypt)
{
   char ip[IP_SIZE];
...
void videoChatSnapshot()
{
    ...
    if(m_CVideoChat)
    {
      ...
      return   m_CVideoChat->videoChatSnapshot(m_VideoChatDelegate );
